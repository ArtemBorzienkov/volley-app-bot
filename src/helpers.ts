...
export type ChatIdTraining = Record<
  string,
  ITraining
>;
...
export const getKeyBoardOption = (
  date: string,
  value: number,
): IKeyBoardOpt => ({
...
export const getTrainingDate = (
  daysInFuture: number,
) => {
  const tomorow = new Date(
    new Date().setDate(
      new Date().getDate() + daysInFuture,
    ),
  );
...
export const getMembersMsg = (
  members: IUser[],
  max: number,
) =>
  `Записані: ${members.map(
    (m, index) =>
